    ...
    <div class="card custom--card">
        <div class="card-header card-header-bg d-flex justify-content-between align-items-center flex-wrap">
            ...
            @if ($myTicket->status != Status::TICKET_CLOSE && $myTicket->user)
                <button class="btn btn--danger close-button btn--sm confirmationBtn" data-question="@lang('Are you sure to close this ticket?')" data-action="{{ route('ticket.close', $myTicket->id) }}" type="button"><i class="la la-times-circle"></i>
                </button>
            @endif
        </div>
        <div class="card-body">
            <form method="post" action="{{ route('ticket.reply', $myTicket->id) }}" enctype="multipart/form-data">
                ...
                <button class="btn btn--base w-100" type="submit"> <i class="la la-reply"></i> @lang('Reply')</button>
            </form>
        </div>
    </div>
        ...
        <div class="card-body">
            @foreach ($messages as $message)
                @if ($message->admin_id == 0)
                    <div class="row border--base m-2 py-2">
                ...
                @else
                    <div class="row border--warning m-2 py-2">
                        <div class="col-md-3 border-end text-end">
...
@push('script')
    <script>
        (function($) {
            ...
            $('.addFile').on('click', function() {
                ...
                $("#fileUploadsContainer").append(`
                    <div class="input-group my-3">
                        ...
                        <button type="submit" class="input-group-text btn--danger border-0 remove-btn"><i class="las la-times"></i></button>
                    </div>
                `)
            });
            ...
            $('.btn--danger').addClass('text-white')
        })(jQuery);
    </script>
@endpush
