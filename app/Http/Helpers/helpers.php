...
{
    $advertisement = Advertisement::where('status', Status::ENABLE)->inRandomOrder()->first();
    ...
    if ($advertisement) {
        ...
        if ($advertisement->type == 1) {
            $html = '<div class="sidebar-widget mt-4">
                        <a href="' . route('ad.redirect', encrypt($advertisement->id)) . '"target="_blank" class="d-block">
                            <img src="' . getImage(getFilePath('ads') . '/' . @$advertisement->content->image, getFileSize('ads')) . '"class="object-fit--cover">
                        </a>
                    </div>';
        } else {
            $html = '<div class="sidebar-widget ajaxCount mt-4" data-id="' . encrypt($advertisement->id) . '">
                        ' . $advertisement->content->script . '
                    </div>';
        }
        echo $html;
    }
